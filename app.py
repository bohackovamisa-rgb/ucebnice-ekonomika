...
def zakovsky_panel():
    ...
    with tab_ukoly:
        ...
        if not odpovedi_data:
            st.info(
                "💡 **Zatím nemáš uložené žádné odpovědi.** Otevři kapitolu,"
                " vyplň úkol a klikni na *Uložit odpověď*."
            )
        else:
            CELKEM_UKOLU = {
                "Kapitola 1": 15,
            ...
            for kap_nazev in sorted(kapitoly_dict.keys()):
                ...
                procento_cislo = (
                    min((hotovo / celkem), 1.0) if celkem > 0 else 1.0
                )
                procento_zobrazeni = int(procento_cislo * 100)

                with st.expander(
                    f"📚 {kap_nazev} ({hotovo}/{celkem} úkolů) —"
                    f" {procento_zobrazeni} %",
                    expanded=False,
                ):
                    st.progress(
                        procento_cislo,
                        text=(
                            f"Stav: {hotovo} z {celkem} hotovo"
                            f" ({procento_zobrazeni} %)"
                        ),
                    )
                    st.markdown("<br>", unsafe_allow_html=True)

                    slozene_odpovedi = sorted(
                        slozene_odpovedi,
                        key=lambda x: str(x.get("otazka_id")),
                    )
                    for row in slozene_odpovedi:
                        ...
                        nova_odpoved = st.text_area(
                            "Tvá odpověď:",
                            value=stare_text,
                            key=f"panel_in_{kap_nazev}_{otazka_id}",
                        )

                        if st.button(
                            "Uložit změnu 💾",
                            key=f"panel_btn_{kap_nazev}_{otazka_id}",
                        ):
    ...
    with tab_investice:
        ...
        st.write(
            "Zde živě vidíš, jak si vedeš na burze. Pokud chceš nakupovat nebo"
            " prodávat další akcie a kryptoměny, přejdi přímo do obchodní"
            " aplikace."
        )

        muj_nick = username.strip().lower()
        url_sim = (
            "https://skolni-investice-jcu32nvp35ymaymjb9g2k5.streamlit.app/?user="
            f"{muj_nick}&nick={muj_nick}"
        )
        st.link_button(
            "🚀 Přejít do investičního simulátoru (Koupit / Prodat)",
            url_sim,
            type="primary",
        )
        ...
        with st.spinner("Aktualizuji data z burzy... ⏳"):
            try:
                ...
                data_inv = sheet_uziv.get_all_records(
                    value_render_option="UNFORMATTED_VALUE"
                )
                df_inv = pd.DataFrame(data_inv)

                df_zaka = df_inv[
                    df_inv["Nick"].astype(str).str.strip().str.lower()
                    == muj_nick
                ]
                ...
                if not df_zaka.empty:
                    ...
                    for db_col, (nazev, mena) in AKTIVA_MAP.items():
                        ...
                        if ks > 0:
                            try:
                                ...
                                cena_czk = (
                                    cena * kurz_usd if mena == "USD" else cena
                                )
                            except Exception:
                                cena_czk = 0.0
                    ...
                    col_met1.metric(
                        "Celkový majetek", f"{celkovy_majetek:,.2f} Kč"
                    )

                    if zisk_ztrata > 0:
                        col_met2.metric(
                            "Čistý zisk / ztráta", f"+{zisk_ztrata:,.2f} Kč"
                        )
                    else:
                        col_met2.metric(
                            "Čistý zisk / ztráta", f"{zisk_ztrata:,.2f} Kč"
                    ...
                    with col_inf1:
                        st.info(
                            "💵 **Volná hotovost k nákupu:**\n\n`"
                            f"{zustatek:,.2f} Kč`"
                        )
                        st.info(
                            "📈 **Hodnota nakoupených aktiv:**\n\n`"
                            f"{hodnota_aktiv:,.2f} Kč`"
                        )
                    ...
                    with col_inf2:
                        ...
                        if drzena_aktiva:
                            ...
                            st.write(
                                "Zatím nedržíš žádné akcie ani kryptoměny."
                            )
                    ...
                    try:
                        ...
                        if data_trans:
                            ...
                            if not df_trans_zak.empty:
                                sloupce_k_zobrazeni = [
                                    c
                                    for c in df_trans_zak.columns
                                    if c not in ["Nick", "Jmeno"]
                                ]
                                st.dataframe(
                                    df_trans_zak[sloupce_k_zobrazeni],
                        ...
                        st.warning(
                            "Nepodařilo se načíst historii tvých transakcí."
                        )
                else:
                    st.info(
                        "Tvůj účet zatím v simulátoru nemá žádná data (nebo se"
                        " uživatelské jméno neshoduje s databází)."
                    )
            ...
            with tab_login:
                with st.form("login_form", border=False):
                    ...
                    password_input = st.text_input(
                        "Heslo:", type="password", placeholder="Vaše heslo..."
                    )
                    submit = st.form_submit_button(
                        "Vstoupit do učebnice", use_container_width=True
                    )
                        ...
                        try:
                            ...
                            if res.data:
                                ...
                                if str(user.get("password")) == password_input:
                                    ...
                                    st.session_state["user_class"] = user.get(
                                        "trida", ""
                                    )
            ...
            with tab_reg:
                with st.form("reg_form", border=False):
                    reg_jmeno = st.text_input(
                        "Jméno a příjmení:", placeholder="Jan Novák"
                    )
                    reg_username = st.text_input(
                        "Zvolte uživatelské jméno:", placeholder="jan.novak"
                    )
                    reg_password = st.text_input(
                        "Zvolte heslo:", type="password"
                    )
                    reg_code = st.text_input(
                        "Zvací kód třídy od učitele:", placeholder="např. EKO3A"
                    )

                    btn_reg = st.form_submit_button(
                        "Vytvořit žákovský účet 🚀", use_container_width=True
                    )
                    ...
                    if btn_reg:
                        if not (
                            ...
                            try:
                                trida_res = (
                                    supabase.table("tridy")
                                    .select("*")
                                    .eq("kod_tridy", reg_code.strip().upper())
                                    .execute()
                                )
                                if not trida_res.data:
                                    st.error(
                                        "Zadaný kód třídy neexistuje! Požádejte"
                                        " učitele o správný kód."
                                    )
                                else:
                                    nazev_tridy = trida_res.data[0][
                                        "nazev_tridy"
                                    ]
                                    ciste_reg_username = ocisti_username(
                                        reg_username
                                    )
                                    user_check = (
                                        supabase.table("uzivatele")
                                        .select("username")
                                        .eq("username", ciste_reg_username)
                                        .execute()
                                    )

                                    if user_check.data:
                                        st.error(
                                            "Toto uživatelské jméno je již"
                                            " zabrané. Zvolte jiné."
                                        )
                                    else:
                                        new_user = {
                                        ...
                                        st.success(
                                            "Účet byl úspěšně vytvořen! Nyní se"
                                            " můžete přihlásit."
                                        )
                            except Exception as e:
...
with st.sidebar:
    ...
    if st.session_state.get("user_role") == "teacher":
        st.markdown(
            "<div class='sidebar-section-title'>👩‍🏫 UČITELSKÝ PANEL</div>",
            unsafe_allow_html=True,
        )

        if st.button(
            "📊 Správa tříd a odpovědí",
            use_container_width=True,
            type=(
                "primary"
        ...
        if st.button(
            ...
            type=(
                "primary"
                if st.session_state["current_view"] == "Ucitel_Materialy"
                else "secondary"
            ),
        ):
    ...
    st.markdown(
        "<div class='sidebar-section-title'>PŘEHLED A ŽÁK</div>",
        unsafe_allow_html=True,
    )
...
# 2. Učitelský panel
    ...
    with tab_tridy:
        ...
        with st.form("nova_trida_form"):
            ...
            novy_kod = st.text_input(
                "Zvolte Zvací kód pro žáky (např. EKO4B):",
                help="Tento kód zadají žáci při registraci.",
            )
                ...
                if novy_nazev and novy_kod:
                    try:
                        ...
                        check = (
                            supabase.table("tridy")
                            .select("kod_tridy")
                            .eq("kod_tridy", kod_clean)
                            .execute()
                        )
                        if check.data:
                            st.error(
                                "Tento Zvací kód již existuje, zvolte jiný."
                            )
                        else:
                            new_trida = {
                                "kod_tridy": kod_clean,
                            ...
                            st.success(
                                f"Třída '{novy_nazev}' vytvořena! Kód pro žáky:"
                                f" **{kod_clean}**"
                            )
                    except Exception as e:
        ...
        try:
            moje_tridy = (
                supabase.table("tridy")
                .select("*")
                .eq("ucitel_username", st.session_state["username"])
                .execute()
            )
            if moje_tridy.data:
                for t in moje_tridy.data:
                    st.info(
                        f"📍 **{t['nazev_tridy']}** | Zvací kód pro žáky:"
                        f" `{t['kod_tridy']}`"
                    )
            else:
    ...
    with tab_vysledky:
        ...
        try:
            ...
            list_trid = (
                [x["nazev_tridy"] for x in t_res.data] if t_res.data else []
            )

            if list_trid:
                vybrana_t = st.selectbox(
                    "Vyberte třídu:", list_trid, key="sel_trida_uc"
                )
                zaci_res = (
                    supabase.table("uzivatele")
                ...
                if zaci_res.data:
                    ...
                    vsechny_odp_res = (
                        supabase.table("odpovedi")
                        .select("username, kapitola")
                        .in_("username", usernames_tridy)
                        .execute()
                    )
                    ...
                    if vsechny_odp_res.data:
                        ...
                        st.info(
                            "Zatím žádný žák v této třídě neodevzdal odpověď."
                        )
                    ...
                    vybrany_zak_jmeno = st.selectbox(
                        "Vyberte žáka pro zobrazení textů:",
                        list(zaci_dict.keys()),
                        key="sel_zak_uc",
                    )
                    ...
                    if odpovedi_res.data:
                        ...
                        st.download_button(
                            label=(
                                f"📥 Stáhnout odpovědi žáka {vybrany_zak_jmeno}"
                                " v CSV (pro Excel)"
                            ),
                            data=csv_data,
                        ...
                        for kap_nazev in sorted(kapitoly_zaka_dict.keys()):
                            ...
                            with st.expander(
                                f"📘 {kap_nazev} — splněno {hotovo}/{celkem}"
                                f" ({pct_zobrazeni} %)",
                                expanded=False,
                            ):
                        ...
                        st.write(
                            "Tento žák zatím neodevzdal žádné odpovědi ke"
                            " kontrole."
                        )
                else:
                    st.write(
                        "V této třídě zatím nejsou zaregistrovaní žádní žáci."
                    )
            else:
                st.write(
                    "Nejprve vytvořte třídu v záložce 'Správa a tvorba tříd'."
                )
        except Exception as e:
    ...
    with tab_investice:
        st.markdown(
            "### 🏆 Živý žebříček investic a historie obchodů"
        )

        with st.spinner(
            "Stahuji aktuální ceny z burzy a data ze simulátoru... ⏳"
        ):
            try:
                t_res = (
                    supabase.table("tridy")
                ...
                if moje_tridy_seznam:
                    zaci_res = (
                        supabase.table("uzivatele")
                        .select("username")
                        .in_("trida", moje_tridy_seznam)
                        .execute()
                    )
                    if zaci_res.data:
                        moje_usernames = [
                            z["username"].strip().lower() for z in zaci_res.data
                        ]
                ...
                if not moje_usernames:
                    ...
                    vsechna_data_zaci = soubor.sheet1.get_all_records(
                        value_render_option="UNFORMATTED_VALUE"
                    )
                    vsechny_transakce = soubor.worksheet(
                        "Transakce"
                    ).get_all_records(value_render_option="UNFORMATTED_VALUE")
                    ...
                    if not df_zaci.empty:
                        df_zaci["Nick_lower"] = (
                            df_zaci["Nick"].astype(str).str.strip().str.lower()
                        )
                        df_zaci = df_zaci[
                            df_zaci["Nick_lower"].isin(moje_usernames)
                        ]
                    ...
                    if not df_zaci.empty:
                        ...
                        for index, row in df_zaci.iterrows():
                            ...
                            for db_col, (nazev, mena) in AKTIVA_MAP.items():
                                ks = float(row.get(db_col, 0.0))
                                if ks > 0:
                                    hodnota_aktiv += ks * zive_ceny.get(
                                        db_col, 0.0
                                    )
                                    drzena_aktiva[nazev] = ks
                        ...
                        zaci_list = sorted(
                            zaci_list,
                            key=lambda x: x["CelkovyMajetek"],
                            reverse=True,
                        )

                        st.markdown("#### 🥇 Žebříček mých studentů")
                        st.write(
                            "Kliknutím na žáka rozbalíte detail jeho portfolia"
                            " a historii nákupů/prodejů."
                        )
                        ...
                        for i, zak in enumerate(zaci_list):
                            ...
                            medaile = (
                                ...
                                if poradi == 1
                                    ...
                                    "🥈"
                                    if poradi == 2
                                    else "🥉" if poradi == 3 else f"{poradi}."
                                )
                            )
                            zisk_str = (
                                f"+{zak['Zisk']:,.2f} Kč"
                                if zak["Zisk"] > 0
                                else f"{zak['Zisk']:,.2f} Kč"
                            )

                            nadpis_expanderu = (
                                f"{medaile} {zak['Jmeno']} (Nick:"
                                f" {zak['Nick']}) — Majetek:"
                                f" {zak['CelkovyMajetek']:,.2f} Kč | Zisk:"
                                f" {zisk_str}"
                            )
                            ...
                            with st.expander(nadpis_expanderu):
                                ...
                                with c1:
                                    ...
                                    st.info(
                                        "💵 Volná hotovost k nákupu: `"
                                        f"{zak['Zustatek']:,.2f} Kč`\n\n📈"
                                        " Hodnota nakoupených akcií: `"
                                        f"{zak['HodnotaAktiv']:,.2f} Kč`"
                                    )
                                with c2:
                                    ...
                                    if zak["Portfolio"]:
                                        for (
                                            aktivum,
                                            kusy,
                                        ) in zak["Portfolio"].items():
                                            st.write(
                                                f"⚡ {aktivum}: `{kusy} ks`"
                                            )
                                    else:
                                        st.write(
                                            "Tento žák zatím nedrží žádné"
                                            " akcie ani kryptoměny."
                                        )

                                st.write("**📜 Historie obchodů:**")
                                if not df_transakce.empty:
                                    df_trans_zak = df_transakce[
                                        df_transakce["Nick"]
                                        .astype(str)
                                        .str.strip()
                                        .str.lower()
                                        == zak["Nick"].strip().lower()
                                    ]
                                    if not df_trans_zak.empty:
                                        sloupce_k_zobrazeni = [
                                            c
                                        ...
                                        st.write(
                                            "Tento žák zatím neprovedl žádný"
                                            " obchod."
                                        )
                                else:
                                    st.write(
                                        "Tabulka historie obchodů je zcela"
                                        " prázdná."
                                    )

                    else:
                        st.info(
                            "Vaši žáci zatím nemají založené účty v"
                            " simulátoru."
                        )
                ...
                * 📈 Praktická kalkulační tabulka (XLSX)
                ...
                try:
                    with open("Influencer - podnikání.zip", "rb") as file:
                        st.download_button(
                            label=(
                                "📥 Stáhnout balíček: Influencer jako firma"
                                " (ZIP)"
                            ),
                            data=file,
                    ...
                    st.warning(
                        "Soubor 'Influencer - podnikání.zip' nebyl na GitHubu"
                        " nalezen."
                    )

        else:
            st.info(
                f"Pro **{vybrana_kap}** zatím nebyly nahrány žádné metodické"
                " balíčky."
            )
...
# 5. Načítání konkrétních kapitol (1 až 6)
    ...
    if st.session_state["current_view"] in kapitoly_map:
        ...
        if st.session_state.get("username"):
            kap_nazev = (
                f"Kapitola {kap_num}"
                if not str(kap_num).startswith("Kapitola")
                else kap_num
            )
