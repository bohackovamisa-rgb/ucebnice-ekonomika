...
def render_dluhovy_simulator():
    ...
    st.write(
        "⚠️ **Důležité upozornění:** V realitě státní dluh neroste takto plynule"
        " každou vteřinou. Roste **skokově** – například když Ministerstvo"
        " financí uspořádá aukci, vydá dluhopisy a půjčí si desítky miliard"
        " najednou. Tento simulátor ale matematicky rozpočítává schodek na"
        " vteřiny, abys měl/a představu, jaké obrovské tempo to v průměru je."
    )
    ...
    st.write(
        "Schodek se každý rok mění. Zkus změnit vládní strategii a sleduj, jak"
        " se počítadlo dluhu zrychlí nebo zpomalí!"
    )
    ...
    scenar = st.selectbox(
        ...
        [
            ...
            (
                "✂️ Rozpočtová odpovědnost / Úspory (Schodek klesne o"
                " polovinu)"
            ),
    ...
    if "Stávající" in scenar:
        ...
        rocni_schodek = st.number_input(
            "Zadej vlastní roční schodek na další roky (v mld. Kč):",
            value=int(aktualni_schodek_mld),
            step=10,
        )
    ...
    roky = st.slider(
        "Simuluj vývoj dluhu za kolik let:", min_value=1, max_value=10, value=4
    )
    ...
    budouci_dluh_poplatnik = (
        budouci_dluh_mld * 1_000_000_000
    ) / EKONOMICKY_AKTIVNI

    st.markdown(
        f"##### 📊 Výsledek za {text_roky} při schodku"
        f" {rocni_schodek:,.0f} mld. Kč ročně:"
    )

    c1, c2, c3 = st.columns(3)
    c1.metric(
        "Celkový budoucí dluh",
        f"{budouci_dluh_mld:,.0f} mld. Kč".replace(",", " "),
    )
    c2.metric(
        "Dluh na 1 občana", f"{budouci_dluh_obcan:,.0f} Kč".replace(",", " ")
    )
    c3.metric(
        "Dluh na pracujícího",
        f"{budouci_dluh_poplatnik:,.0f} Kč".replace(",", " "),
    )

    st.info(
        "💡 **Shrnutí:** Pokud bude stát hospodařit v tomto zvoleném režimu,"
        f" naroste dluh za **{text_roky}** o **{rocni_schodek * roky:,.0f} mld."
        " Kč**. Na každého ekonomicky aktivního občana (toho, kdo reálně platí"
        " daně ze mzdy) tak v budoucnu připadne dluhová zátěž"
        f" **{budouci_dluh_poplatnik:,.0f} Kč**."
    )
...
def render():
    ...
    st.markdown("## 5. Stát, daně a globální souvislosti")
    st.markdown(
        "<p style='font-size: 1.1rem; color: #64748b; margin-bottom: 1.5rem;'>"
        "Stát jako hospodář: daně, veřejné služby a propojený svět<br><br>"
        ...
        with c_nav1:
            ...
            * diskutovat ekonomická dilemata bez jednoduchých odpovědí.
            ...
            <div style="font-size: 0.85rem; color: #64748b; margin-top: 10px;">
                <i>📘 <b>Vazba na RVP:</b> Kapitola rozvíjí ekonomické, občanské a digitální kompetence v oblastech funkce státu v ekonomice, hospodářská politika, daňová soustava, státní rozpočet, veřejné finance, globalizace, mezinárodní obchod, Evropská unie, udržitelný rozvoj a odpovědné rozhodování jednotlivce i firmy.</i>
            </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    if selected_section_5.startswith("1."):
        ...
        st.markdown(
            """
        <div class='box-green'>
            🎯 <b>Cíl 1. bloku:</b> Pochopíš, proč trh sám nevyřeší všechno, jaké funkce má stát v ekonomice, podle čeho se hodnotí úspěšnost hospodářské politiky a jaké nástroje stát používá, když chce ekonomiku stabilizovat.
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        with st.expander(
            ...
            * vysvětlit tržní selhání,
            * rozlišit veřejné statky, externality a monopoly,
            * popsat základní funkce státu v ekonomice,
            * vysvětlit magický čtyřúhelník hospodářské politiky,
            * rozlišit fiskální, monetární a sociální politiku,
            * propojit teorii s běžným životem: Vinted, online nákupy, hypotéky, inflace, fast fashion, zdravotnictví, důchody a Big Tech.
        ...
        st.write(
            "Trh je silný nástroj. Umí propojit nabídku a poptávku, motivovat"
            " firmy ke zlepšování, tlačit na cenu a umožnit lidem svobodně se"
            " rozhodovat. Jenže trh není kouzelný algoritmus, který automaticky"
            " vyřeší všechno spravedlivě, bezpečně a dlouhodobě udržitelně."
        )
        st.write(
            "**Tržní selhání** znamená situaci, kdy samotný trh nevede k"
            " výsledku, který je pro společnost efektivní nebo přijatelný."
            " Stát proto nastavuje pravidla, financuje některé služby, omezuje"
            " škodlivé dopady a chrání hospodářskou soutěž."
        )

        st.markdown(
            "<div class='box-blue'><strong>🧠 Jednoduše:</strong> Stát"
            " nezasahuje do ekonomiky proto, že by trh byl zbytečný. Zasahuje"
            " tam, kde trh sám vytváří problém, který dopadá na celou"
            " společnost.</div>",
            unsafe_allow_html=True,
        )

        tab_selhani1, tab_selhani2, tab_selhani3 = st.tabs([
            "🛣️ Veřejné statky (Kickstarter)",
            "🏭 Externality (Levné tričko)",
            "👑 Monopol (Mobilní operátor)",
        ])

        with tab_selhani1:
            st.markdown(
                "#### Proč si silnice, lampy a armádu neplatíme přes"
                " Kickstarter? (RVP pojem: veřejné statky)"
            )
            st.write(
                "Veřejné statky jsou statky nebo služby, u kterých obvykle"
                " platí dvě vlastnosti:"
            )
            st.markdown("""
            ...
            * **Příklady:** Veřejné osvětlení, obrana státu, základní bezpečnost, některé silnice, protipovodňová ochrana, čistší veřejný prostor.
            * **Problém černého pasažéra:** Kdyby se veřejné statky platily jen dobrovolně, mnoho lidí by je využívalo, ale nepřispělo by. Proto je často financuje stát z daní.
            """)

        with tab_selhani2:
            st.markdown(
                "#### Proč levné tričko nemusí být opravdu levné? (RVP pojem:"
                " externality)"
            )
            st.write(
                "**Externalita** je vedlejší efekt výroby nebo spotřeby, který"
                " dopadá na někoho, kdo se přímo neúčastní tržní transakce."
            )
            st.markdown("""
            * 🔴 **Negativní externalita:** Firma něco vyrábí levně, ale část nákladů nese okolí — například znečištěná řeka, emise, hluk, přetížená doprava nebo špatné pracovní podmínky v dodavatelském řetězci.
            * 👗 **Moderní příklad:** Fast fashion může mít nízkou cenu na účtence, ale část nákladů se projeví jinde: v odpadu, spotřebě vody, emisích z dopravy nebo pracovních podmínkách.
            * 🟢 **Pozitivní externalita:** Vzdělání prospívá nejen jednotlivci, ale celé společnosti. Vzdělanější člověk může mít vyšší příjem, platit vyšší daně, lépe se rozhodovat, inovovat a přispívat komunitě. Proto stát podporuje školství.
            """)

        with tab_selhani3:
            st.markdown(
                "#### Co by se stalo, kdyby byl v Česku jen jeden mobilní"
                " operátor? (RVP pojem: monopol a nedokonalá konkurence)"
            )
            st.write(
                "**Monopol** vzniká tehdy, když jeden podnik ovládá trh natolik,"
                " že může výrazně ovlivňovat cenu, podmínky nebo dostupnost"
                " služby. Problémem není velikost firmy sama o sobě, ale"
                " **zneužití tržní síly**."
            )
            st.markdown("""
            * **Rizika monopolu:** vyšší ceny, horší kvalita, menší tlak na inovace, slabší postavení zákazníka.
            * **Moderní příklady:** Debaty o Big Tech firmách, uzavřených ekosystémech, povinném USB-C, přístupu k aplikacím, pravidlech platforem nebo férové soutěži v digitálním prostředí.
            * **Role státu a EU:** Chránit hospodářskou soutěž, bránit zneužívání dominantního postavení a chránit spotřebitele.
            """)

        st.markdown(
            "<div class='box-yellow'><strong>❓ Debata:</strong> Kde je hranice"
            " mezi užitečnou regulací a zbytečnou byrokracií? Má stát zasahovat"
            " do digitálních platforem, AI nástrojů, mobilních ekosystémů nebo"
            " pravidel e-shopů?</div>",
            unsafe_allow_html=True,
        )

        st.divider()
        st.markdown("### 1.2 Co přesně stát v ekonomice dělá: funkce státu")
        st.write(
            "Ekonomická role státu se dá shrnout do několika funkcí. Neznamená"
            " to, že stát má řídit každou cenu a každé podnikání. Znamená to,"
            " že vytváří rámec, ve kterém může trh fungovat, a zasahuje tam,"
            " kde samotný trh nestačí."
        )

        st.markdown("""
        | Funkce státu | RVP vysvětlení | Moderní příklad |
        | :--- | :--- | :--- |
        | **Právní a institucionální rámec** | Stát zajišťuje pravidla hry: ochranu vlastnictví, smlouvy, vymahatelnost práva, ochranu spotřebitele a základní instituce. | Když si objednáš zboží online a přijde padělek nebo nic, existují pravidla reklamace, ochrana spotřebitele, ČOI, soudy a možnost bránit se. |
        | **Alokační funkce** | Stát směřuje zdroje tam, kde je trh sám neposkytuje v dostatečné míře, hlavně k veřejným statkům a službám. | Školy, nemocnice, silnice, hasiči, veřejné osvětlení nebo protipovodňová opatření. |
        | **Redistribuční funkce** | Stát zmírňuje sociální nerovnosti pomocí daní, dávek, důchodů, podpory v nezaměstnanosti nebo veřejných služeb. | Debata, zda mají lidé s vyššími příjmy platit vyšší daně, aby stát mohl financovat obědy ve školách, dostupnější bydlení nebo sociální pomoc. |
        | **Stabilizační funkce** | Stát se snaží tlumit výkyvy ekonomiky, inflaci, nezaměstnanost a hluboké krize. | Podpora během pandemie (Kurzarbeit), pomoc domácnostem při energetické krizi, snaha brzdit inflaci nebo podporovat zaměstnanost. |
        """)

        st.info(
            "🧭 **Pravidla hry:** Bez vymahatelných pravidel by trh nefungoval"
            " dobře. Když nevíš, zda smlouva platí, zda můžeš reklamovat zboží,"
            " zda někdo chrání tvoje vlastnictví a zda firma může lhát v"
            " reklamě, ekonomika se mění v chaos."
        )
        st.warning(
            "⚖️ **Redistribuce není jen „brát a dávat“:** Smyslem je řešit"
            " situace, kdy by příliš velké nerovnosti ohrožovaly soudržnost"
            " společnosti, přístup ke vzdělání, zdraví nebo základní"
            " důstojnost. Zároveň ale každá redistribuce otevírá debatu o"
            " motivaci pracovat, spravedlnosti a odpovědnosti."
        )
        ...
        with st.expander("Debatní otázka: Mají miliardáři platit vyšší daně?"):
            ...
            * **Argumenty proti:** riziko odchodu kapitálu, menší motivace investovat, složitější daňový systém, otázka hranice „spravedlivého“ zdanění.
            * **Výstup:** Napiš své stanovisko ve 4 větách: názor, ekonomický argument, sociální argument, možné riziko.
            """)

        st.divider()
        st.markdown(
            "### 1.3 Ukazatele výkonu ekonomiky a magický čtyřúhelník"
        )
        st.write(
            "Aby stát, vláda, ČNB, firmy i občané věděli, v jaké kondici"
            " ekonomika je, sledují se **makroekonomické ukazatele**. Jsou to"
            " čísla, která pomáhají odpovědět na otázky: Roste ekonomika? Mají"
            " lidé práci? Nezdražuje se příliš rychle? Jak jsme na tom vůči"
            " zahraničí?"
        )
        st.write(
            "Nejde o čísla „pro ekonomy do tabulek“. Když se změní inflace,"
            " nezaměstnanost, úrokové sazby nebo HDP, projeví se to v cenách"
            " potravin, dostupnosti brigád, mzdách, hypotékách, investicích"
            " firem i náladě ve společnosti."
        )
        ...
        * **Reálná mzda** — ukazuje, zda mzdy rostou rychleji, nebo pomaleji než ceny.
        ...
        st.markdown(
            "<div class='box-green'><strong>🔗 Klikací zdroje aktuálních dat"
            " pro žáky:</strong><br>- <a"
            " href='https://csu.gov.cz/produkty/hmu_cr' target='_blank'>ČSÚ —"
            " Hlavní makroekonomické ukazatele</a><br>- <a"
            " href='https://csu.gov.cz/inflace-spotrebitelske-ceny'"
            " target='_blank'>ČSÚ — Inflace, spotřebitelské ceny</a><br>- <a"
            " href='https://www.cnb.cz/cs/statistika/inflace/'"
            " target='_blank'>ČNB — Inflace</a><br>- <a"
            " href='https://www.cnb.cz/cs/statistika/platebni_bilance_stat/'"
            " target='_blank'>ČNB — Statistiky platební bilance</a></div>",
            unsafe_allow_html=True,
        )

        st.markdown(
            "<div class='box-yellow'><strong>🧩 Úkol s reálnými daty:</strong>"
            " Otevři odkaz na ČSÚ — Hlavní makroekonomické ukazatele. Najdi"
            " poslední dostupný rok a opiš do sešitu nebo tabulky: růst HDP,"
            " průměrnou roční inflaci, obecnou míru nezaměstnanosti a saldo"
            " běžného účtu platební bilance k HDP. Potom napiš jednu větu:"
            " „Podle těchto čísel ekonomika vypadá spíš stabilně / nestabilně,"
            " protože…“</div>",
            unsafe_allow_html=True,
        )
        ...
        st.write(
            ...
            " všechny cíle nejdou dokonale splnit současně."
        )
        st.write(
            "Když se stát snaží rychle podpořit ekonomiku, může tím snížit"
            " nezaměstnanost, ale zároveň zvýšit inflaci. Když se naopak snaží"
            " inflaci brzdit, může zpomalit růst a ztížit firmám investice."
            " Proto hospodářská politika není jednoduché tlačítko „zlepšit"
            " ekonomiku“, ale hledání rovnováhy mezi několika cíli."
        )
        ...
        **Jak si představit magický čtyřúhelník:**
        ...
        st.info(
            "📈 **Jak nákres číst:** Čím vyrovnanější jsou všechny čtyři"
            " oblasti, tím zdravěji ekonomika působí. Pokud je jeden vrchol"
            " výrazně „rozbitý“, celá ekonomika může být pod tlakem. Čím blíže"
            " je ekonomika k příznivým hodnotám ve všech čtyřech směrech, tím"
            " vyrovnanější je její výkon. Problém je, že zlepšení jedné oblasti"
            " může zhoršit jinou."
        )
        ...
        st.markdown(
            ...
            " neviditelný zloděj úspor</strong><br>Zeptej se doma nebo ve"
            " třídě: Kolik stál kopeček zmrzliny, kebab, lístek do kina nebo"
            " školní oběd před pěti lety? Kolik stojí dnes? Pokud ceny rostou"
            " rychleji než příjmy a úspory, člověk si za stejné peníze koupí"
            " méně.</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander("Proč nejde mít všechno najednou?"):
            st.write(
                "Když stát výrazně zvyšuje výdaje nebo snižuje daně, může tím"
                " podpořit poptávku, firmy a zaměstnanost. Pokud je ale"
                " ekonomika už napnutá, může více peněz v oběhu tlačit na růst"
                " cen. Naopak přísná politika proti inflaci může zdražit úvěry,"
                " zpomalit investice a dočasně zvýšit nezaměstnanost."
            )
            st.write(
                "**Příklad:** Nižší úrokové sazby mohou pomoci lidem a firmám"
                " levněji si půjčit. To může podpořit spotřebu, investice a"
                " pracovní místa. Pokud je ale v ekonomice příliš mnoho poptávky"
                " a málo zboží nebo služeb, ceny mohou růst rychleji. Jeden cíl"
                " se tedy může zlepšit, zatímco jiný se zhorší."
            )

        st.info(
            "📘 **Co je důležité pro RVP:** Magický čtyřúhelník není jen"
            " obrázek. Je to způsob, jak přemýšlet o hospodářské politice: každé"
            " rozhodnutí má dopady na růst, zaměstnanost, ceny i vztahy se"
            " zahraničím."
        )
        ...
        st.write(
            "Hospodářská politika je soubor rozhodnutí a nástrojů, kterými stát"
            " a veřejné instituce ovlivňují ekonomiku. Nejde jen o vládu."
            " Důležitou roli má také centrální banka, samosprávy, zákony,"
            " regulace a systém sociální ochrany."
        )
        ...
        st.info(
            "🏦 **ČNB a vláda nejsou totéž:** Vláda rozhoduje hlavně o"
            " rozpočtu, daních a výdajích. ČNB je nezávislá centrální banka,"
            " která nastavuje měnovou politiku. Obě oblasti ale ovlivňují"
            " stejnou ekonomiku, proto spolu souvisejí."
        )

        with st.expander("Proč je těžší dosáhnout na hypotéku?"):
            st.write(
                "Když jsou úrokové sazby vyšší, půjčené peníze jsou dražší."
                " Hypotéka má vyšší měsíční splátku, banka přísněji posuzuje"
                " schopnost splácet a část lidí na úvěr nedosáhne. Vyšší sazby"
                " mohou brzdit inflaci, ale zároveň komplikují bydlení,"
                " investice firem i spotřebu domácností."
            )

        with st.expander(
            "Co se stane, když onemocníš, ztratíš práci nebo zestárneš?"
        ):
            st.write(
                "To řeší sociální politika a systém sociálního zabezpečení. V ČR"
                " existuje veřejné zdravotní pojištění, nemocenské pojištění,"
                " důchodový systém, podpora v nezaměstnanosti a různé sociální"
                " dávky. Smyslem je, aby životní krize automaticky neznamenala"
                " úplný finanční kolaps."
            )
            st.write(
                "**Srovnání pro pochopení:** V některých zemích může vážnější"
                " zdravotní problém znamenat vysoké účty a riziko zadlužení. V"
                " ČR člověk u běžné péče často ukáže kartičku pojišťovny,"
                " protože zdravotnictví je financováno převážně přes veřejné"
                " zdravotní pojištění. Není to „zdarma“ — platí se průběžně z"
                " odvodů a veřejných peněz."
            )

        with st.expander(
            "Debata: Bude důchodový systém stačit i pro dnešní středoškoláky?"
        ):
            st.write(
                "Jak se změní důchodový systém, když populace stárne a méně"
                " pracujících lidí vydělává na rostoucí počet seniorů? Jakou"
                " roli v tom hraje vlastní finanční příprava a investice?"
            )

        st.divider()
        st.markdown("### 1.5 Kvíz: poznáš funkci státu?")
        st.write(
            "U každé situace urči, o jakou funkci nebo nástroj státu jde."
            " Nejdřív odpověz sám/sama, potom zkontroluj řešení:"
        )
        ...
        with st.form("fce_statu_quiz_v3"):
            ...
            q5 = st.selectbox(
                "5. Stát podporuje očkování nebo vzdělávání, protože přínos má"
                " i okolí a celá společnost.",
                [
                    "Vyber...",
            ...
            if st.form_submit_button("Zkontrolovat a uložit mé odpovědi 💾"):
                if (
                    ...
                    st.success(
                        ...
                        " funkce a nástroje stát v ekonomice má."
                    )
                else:
                    st.error(
                        "❌ Některá z odpovědí je chybná. \n\n*Správné řešení:"
                        " 1. Redistribuční funkce (přerozdělení). 2. Alokační"
                        " funkce (veřejná infrastruktura). 3. Monetární"
                        " politika (stabilizace cen). 4. Právní a"
                        " institucionální rámec (ochrana spotřebitele). 5."
                        " Pozitivní externalita (přínos pro společnost).*"
                    )
        ...
        if "vykresli_otazku_fn" in st.session_state:
            ...
            st.session_state["vykresli_otazku_fn"](
                ...
                " alokační, redistribuční, stabilizační nebo pravidla hry.",
                "5",
                st.session_state.get("ulozene_odpovedi", {}),
            )
            st.session_state["vykresli_otazku_fn"](
                "5.1.4",
                "3. Vysvětli, co by se stalo, kdyby o všem rozhodoval jen"
                " „trh“ — tedy kdo zaplatí, ten má službu/pomoc, kdo"
                " nezaplatí, nemá nic.",
                "5",
                st.session_state.get("ulozene_odpovedi", {}),
            )
    ...
    # =========================================================================
        ...
        st.markdown(
            "<div class='box-green'>"
            "🧾 <b>Moderní hook:</b> Kolik státu odevzdáš z první brigády,"
            " prodeje na Vinted, streamování na Twitchi, spolupráce na TikToku"
            " nebo zisku z kryptoměn? Daně nejsou jen formulář. Jsou to"
            " pravidla, která propojují soukromé příjmy, veřejné služby, státní"
            " rozpočet, solidaritu i ekonomickou odpovědnost."
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "<div class='box-blue'>"
            "🎯 <b>Cíl 2. bloku:</b> Pochopíš, co je daň, poplatek a clo, jak"
            " se liší přímé a nepřímé daně, co stát financuje z rozpočtu, proč"
            " vzniká deficit a státní dluh a kde je hranice mezi legální"
            " daňovou optimalizací a nelegálním daňovým únikem."
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander(
            ...
            st.markdown("""
            **Co se v bloku naučíš:**
            * vysvětlit pojem daň, poplatek a clo,
            * popsat fiskální, alokační a redistribuční funkci daní,
            * rozlišit zásady zdaňování: spravedlnost, efektivnost a jednoduchost,
            * porovnat progresivní a rovnou daň,
            * rozlišit přímé a nepřímé daně,
            * vysvětlit DPFO, DPPO, DPH, spotřební daně, ekologické daně a vybrané majetkové daně,
            * propojit daně s brigádou, DPP/DPČ, online příjmy, Vinted, investicemi a kryptoměnami,
            * popsat příjmy, výdaje, deficit a dluh státu,
            * vysvětlit rozdíl mezi optimalizací, únikem a stínovou ekonomikou.

            **Vazba na RVP pro SŠ:**  
            Tento blok pokrývá daňovou soustavu, státní rozpočet, příjmy a výdaje státu, hospodaření veřejného sektoru, sociální solidaritu, odpovědné finanční rozhodování, práva a povinnosti občana, základní orientaci v pracovních příjmech, podnikání, spotřebě a ekonomické realitě státu.
            """)
        ...
        st.write(
            ...
            " protislužbu. Neplatíš daň z příjmu proto, aby ti stát druhý den"
            " opravil přesně tvoji ulici. Platíš ji do společného systému, ze"
            " kterého se financují veřejné služby a fungování státu."
        )

        st.markdown(
            "<div class='box-yellow'><strong>🧠 Jednoduše:</strong> Daň je"
            " povinný příspěvek do společné peněženky státu, krajů nebo obcí. Z"
            " této peněženky se platí věci, které společnost potřebuje: školy,"
            " silnice, bezpečnost, soudy, veřejnou správu, sociální systém, část"
            " zdravotnictví, obranu nebo pomoc v krizích.</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander(
            "Daň vs. poplatek: proč daň z příjmu není totéž co poplatek za psa?"
        ):
            st.write(
                "**Daň z příjmu** platíš podle toho, že máš zdanitelný příjem."
                " Peníze jdou do veřejných rozpočtů a stát je použije na"
                " široké spektrum výdajů."
            )
            st.write(
                "**Poplatek za psa** nebo poplatek za komunální odpad má užší"
                " vazbu na konkrétní místní správu, evidenci, službu nebo náklad"
                " obce."
            )
            st.info(
                "📌 **Pointa:** Daň je obecnější povinný příspěvek. Poplatek je"
                " konkrétnější platba za určitou službu, evidenci nebo úkon."
            )

        st.divider()
        st.markdown("### 2.2 Funkce daní: proč stát daně vybírá")
        st.write(
            "Daně nemají jen jednu roli. V ekonomice plní několik funkcí:"
        )

        st.markdown("""
        | Funkce daní | Co znamená | Příklad |
        | :--- | :--- | :--- |
        | **Fiskální funkce** | Daně přinášejí peníze do veřejných rozpočtů. | Z DPH, daně z příjmů a dalších daní stát financuje školy, bezpečnost, úřady, důchody nebo infrastrukturu. |
        | **Alokační funkce** | Daně pomáhají financovat oblasti, které by trh sám neposkytl dostatečně. | Veřejné školství, silnice, hasiči, obrana, výzkum nebo ochrana životního prostředí. |
        | **Redistribuční funkce** | Daně a veřejné výdaje mohou zmírňovat nerovnosti. | Lidé s vyššími příjmy přispívají více a stát financuje dávky, důchody, veřejné služby nebo podporu slabších skupin. |
        | **Regulační funkce** | Daně mohou motivovat nebo odrazovat od určitého chování. | Spotřební daň u alkoholu, tabáku a paliv; ekologické daně nebo možné daně ze sladkých nápojů. |
        """)

        st.warning(
            "⚖️ **Dilema daní:** Když jsou daně příliš nízké, stát nemusí mít"
            " dost peněz na služby a investice. Když jsou příliš vysoké nebo"
            " složité, mohou odrazovat od práce, podnikání, investic nebo vést"
        ...
        st.write(
            "Dobrá daň by měla být nejen výnosná, ale také srozumitelná,"
            " spravedlivá a co nejméně škodit ekonomické aktivitě."
        )
        ...
        st.write(
            "Daňový systém může být nastaven různě. Dvě důležité myšlenky jsou"
            " rovná daň a progresivní daň."
        )

        st.markdown("""
        | Typ zdanění | Princip | Výhoda | Riziko |
        | :--- | :--- | :--- | :--- |
        | **Rovná daň** | Všichni platí stejné procento ze základu daně. | Je jednodušší a přehlednější. | Může být vnímána jako méně solidární, protože nízkopříjmové domácnosti cítí každou korunu výrazněji. |
        | **Progresivní daň** | S vyšším příjmem roste daňová sazba nebo daňové zatížení. | Více zohledňuje schopnost platit. | Může vyvolat debatu o motivaci pracovat, investovat a podnikat. |
        """)

        with st.expander(
            "Debata: Měl by člověk s příjmem 200 000 Kč měsíčně platit vyšší"
            " procento než člověk s 30 000 Kč?"
        ):
            st.markdown("""
            * **Argument pro progresi:** Člověk s vysokým příjmem má větší schopnost přispět a i po vyšší dani mu zůstává více prostředků. Progrese může pomoci financovat školství, zdravotnictví nebo sociální pomoc.
            * **Argument proti progresi:** Příliš vysoké zdanění může působit demotivačně, podporovat daňovou optimalizaci nebo odchod části příjmů do zahraničí.
            * **Výstup:** Napiš krátké stanovisko: Jaká daň je podle tebe spravedlivá a proč?
            """)

        with st.expander("Mýtus vs. realita progresivní daně"):
            st.write(
                "**Mýtus:** „Když přejdu do vyššího daňového pásma, vydělám ve"
                " výsledku méně peněz.“\n\n**Realita:** U běžné progresivní daně"
                " se vyšší sazbou obvykle daní až *část příjmu nad určitou"
                " hranicí*, ne celý příjem. Člověk tedy po zvýšení hrubého"
                " příjmu zpravidla nemá méně peněz než předtím — jen z"
                " dodatečné části příjmu odvede vyšší podíl."
            )

        st.divider()
        st.markdown(
            "### 2.5 Přímé daně: peníze, které se vážou ke konkrétní osobě nebo"
            " firmě"
        )
        st.write(
            "Přímé daně platí konkrétní člověk nebo firma ze svého příjmu, zisku"
            " nebo majetku. Daň je tedy přímo spojena s poplatníkem."
        )

        st.markdown("""
        | Přímá daň | Kdo ji platí | Co zdaňuje | Příklad |
        | :--- | :--- | :--- | :--- |
        | **Daň z příjmů fyzických osob (DPFO)** | Jednotlivci. | Mzdu, podnikání, některé pronájmy, kapitálové příjmy a další zdanitelné příjmy. | Brigáda, zaměstnání, OSVČ, některé online příjmy. |
        | **Daň z příjmů právnických osob (DPPO)** | Firmy a jiné právnické osoby. | Zisk právnické osoby. | S.r.o. platí daň ze zisku. |
        | **Daň z nemovitých věcí** | Vlastníci pozemků, domů, bytů nebo dalších nemovitostí podle pravidel zákona. | Vlastnictví nemovitosti. | Majitel bytu nebo domu platí daň obci/státu podle typu a místa nemovitosti. |
        | **Silniční daň** | U vybraných vozidel podle zákonných pravidel. | Používání určitých vozidel, zejména v podnikatelském kontextu. | Typicky se týká spíše firemních a nákladních vozidel než běžného studenta s osobním autem. |
        """)

        st.info(
            "📌 **Reality check: první brigáda:** U DPP nebo DPČ není důležité"
            " jen „kolik je hodinová mzda“. Záleží na typu dohody, výši příjmu,"
            " odvodech, dani, podepsaném Prohlášení poplatníka a slevách na"
            " dani. Proto hrubá mzda není automaticky částka, která přijde na"
            " účet."
        )

        with st.expander("Co je „růžový papír“?"):
            st.write(
                "**Prohlášení poplatníka k dani** je formulář, díky kterému"
                " může zaměstnanec u jednoho zaměstnavatele uplatnit základní"
                " slevu na poplatníka a případně další slevy podle aktuálních"
                " pravidel."
            )
            st.write(
                "**Pro studenta:** Pokud pracuješ na brigádě, podepsané"
                " prohlášení může výrazně ovlivnit čistou částku na účtu. Nelze"
                " ho ale běžně uplatňovat u více zaměstnavatelů současně za"
                " stejné období."
            )

        with st.expander(
            "Jak se daní TikTok, Twitch, OnlyFans, Patreon nebo barter na"
            " Instagramu?"
        ):
            st.write(
                "Pokud někdo dlouhodobě a soustavně vydělává tvorbou obsahu,"
                " spolupracemi, reklamou, předplatným, dary od fanoušků nebo"
                " prodejem digitálních produktů, nejde jen o „peníze z"
                " internetu“. Může jít o zdanitelný příjem a někdy i o"
                " podnikání."
            )
            st.write(
                "**Barter:** Když influencer dostane produkt nebo službu výměnou"
                " za propagaci, nemusí jít o nulový příjem. I nepeněžní plnění"
                " může mít hodnotu, kterou je potřeba řešit daňově."
            )
            st.info(
                "💡 **Praktické pravidlo:** U online příjmů nestačí říct"
                " „nepřišla mi výplata od firmy“. Důležité je, jestli vznikla"
                " ekonomická hodnota a zda jde o příjem podle daňových pravidel."
            )

        with st.expander("Vinted, Bazoš, eBay: kdy je to ještě prodej vlastních věcí?"):
            st.write(
                "Když prodáš vlastní staré oblečení, telefon nebo učebnice,"
                " obvykle jde o jinou situaci než pravidelný nákup a prodej"
                " zboží za účelem zisku."
            )
            st.write(
                "**Bezpečná otázka:** Prodávám vlastní použité věci, nebo"
                " soustavně nakupuji/prodávám se záměrem vydělávat?"
            )
            st.warning(
                "⚠️ **Pozor:** Přesné limity a osvobození se mohou měnit. U"
                " vyšších částek, pravidelnosti nebo systematického prodeje je"
                " nutné ověřit aktuální pravidla u Finanční správy nebo daňového"
                " poradce."
            )

        with st.expander("Kryptoměny, akcie a ETF: co si ověřit před prodejem?"):
            st.write(
                "U investic je důležité rozlišit typ aktiva, dobu držení, výši"
                " příjmů a aktuální daňová pravidla."
            )
            st.markdown("""
            * U akcií a ETF se často řeší časový test a limity pro osvobození příjmů z prodeje cenných papírů.
            * U kryptoměn se může zdanění lišit od cenných papírů a časový test nemusí fungovat stejně.
            * U NFT, tokenů nebo zahraničních platforem je potřeba pečlivě evidovat nákupy, prodeje, poplatky a kurzy.
            """)
            st.info(
                "📱 **Pravidlo pro studenta:** Aplikace jako Revolut nebo"
                " Trading212 usnadní nákup, ale nezbaví člověka povinnosti"
                " zjistit, jak se řeší daně."
            )
        ...
        with st.form("diskuse_regiony"):
            st.write(
                "Diskusní otázka: Je spravedlivější, aby víc peněz z daní"
                " zůstávalo obcím, kde se vyberou, nebo aby se více"
                " přerozdělovalo mezi bohatší a chudší regiony?"
            )
        ...
        with st.expander("Rozpočtový reality check: kdyby stát byl domácnost?"):
            st.write(
                "Domácnost má povinné platby: nájem, energie, splátky, jídlo."
                " Stát má podobně výdaje, které jsou velmi těžko okamžitě"
                " změnitelné. Rozdíl je v tom, že stát není běžná domácnost:"
                " může vybírat daně, vydávat dluhopisy a rozhodovat o veřejných"
                " službách. Přesto platí, že dlouhodobé deficity vytvářejí tlak"
                " na budoucí rozpočty."
            )

        st.markdown(
            "### 2.9 Vyrovnaný, přebytkový a schodkový rozpočet"
        )
        st.markdown("""
        ...
        st.divider()
        st.markdown(
            "### 2.11 Daňové úniky, optimalizace a stínová ekonomika"
        )
        st.write(
        ...
        with st.expander("Práce „na ruku“: proč je sleva bez účtenky problém?"):
            st.write(
                "Když zákazník zaplatí hotově bez dokladu, může mít pocit, že"
                " ušetřil. Jenže chybí doklad pro reklamaci, příjem nemusí být"
                " zdaněný a pracovník nemusí mít správně odvody. To může"
                " znamenat nižší příjmy veřejných rozpočtů, horší ochranu"
                " zákazníka i dopady na sociální a zdravotní pojištění."
            )

        with st.expander("Jak se vyhýbají daním velké firmy?"):
            st.write(
                "Nadnárodní firmy mohou využívat rozdíly mezi daňovými systémy"
        ...
        st.markdown(
            ...
            " daní:</strong><br>- <a href='https://financnisprava.gov.cz/'"
            " target='_blank'>Finanční správa ČR</a><br>- <a"
            " href='https://adisspr.mfcr.cz/' target='_blank'>Portál MOJE"
            " daně</a><br>- <a"
            " href='https://adisspr.mfcr.cz/pmd/epo/formulare'"
            " target='_blank'>MOJE daně — Elektronické formuláře EPO</a><br>-"
            " <a href='https://www.mpsv.cz/kalkulacka-ciste-mzdy'"
            " target='_blank'>Kalkulačka čisté mzdy MPSV</a></div>",
            unsafe_allow_html=True,
        )
        ...
        st.write(
            ...
            " oblečení na Vinted.\n3. Tvůrce dostane barter za propagaci na"
            " Instagramu.\n4. Někdo prodá kryptoměnu se ziskem.\n5. Opravář"
            " nabídne nižší cenu bez dokladu."
        )
        st.write(
    ...
    # =========================================================================
        ...
        st.markdown(
            "<div class='box-blue'>"
            "💻 <b>Praktický přesah:</b> Daňový portál, datová schránka,"
            " elektronická identita a Portál občana ukazují, že ekonomika není"
            " jen teorie. Moderní občan potřebuje rozumět tomu, kde hledat"
            " informace, jak ověřovat povinnosti a proč je digitální"
            " komunikace se státem součástí finanční gramotnosti."
            "</div>",
            unsafe_allow_html=True,
        )

        st.markdown("### 3.1 Praktické scénáře pro studenty")
        with st.expander(
            "Proč hrubá mzda není to samé, co mi přijde na účet? (První brigáda"
            " a „Růžový papír“)"
        ):
            st.write(
                "U DPP nebo DPČ není důležité jen „kolik je hodinová mzda“."
                " Záleží na typu dohody, výši příjmu, odvodech, dani,"
                " podepsaném **Prohlášení poplatníka k dani** (růžový papír) a"
                " slevách na dani. Díky růžovému papíru může zaměstnanec"
                " uplatnit základní slevu na poplatníka. Lze ho ale uplatňovat v"
                " daném měsíci vždy pouze u jednoho zaměstnavatele."
            )
        ...
        with st.expander(
            ...
            st.write(
                ...
                " prodejem digitálních produktů, nejde jen o „peníze z"
                " internetu“. Může jít o zdanitelný příjem a někdy i o"
                " podnikání. I barter (produkt výměnou za reklamu) může mít"
                " ekonomickou hodnotu a daňové dopady."
            )

        with st.expander(
            "Vinted, Bazoš, eBay: Kdy je to ještě prodej vlastních věcí?"
        ):
            st.write(
        ...
        with st.expander("Kryptoměny, akcie a ETF: Co si ověřit před prodejem?"):
            st.write(
                "U akcií a ETF se často řeší tzv. **časový test** (po určité"
                " době držení je prodej osvobozen od daně) a limit ročních"
                " příjmů z prodeje. U kryptoměn se zdanění liší od běžných"
                " cenných papírů a časový test v ČR na kryptoměny tradičně"
                " neplatil (pravidla se mohou měnit, je třeba sledovat platnou"
                " legislativu). Revolut a další apky usnadní nákup, ale daně"
                " musíš řešit ty."
            )

        st.markdown("### 3.2 Trenažér: „Tohle přece danit nemusím!“")
        st.write(
            "Vyber jednu situaci — brigáda, doučování, prodej výrobků, Vinted,"
            " YouTube/TikTok, pronájem přes Airbnb, investice nebo kryptoměny."
            " Napiš, jaké otázky by sis musel/a ověřit, než prohlásíš: „Tohle"
            " danit nemusím.“"
        )
        ...
        if "vykresli_otazku_fn" in st.session_state:
            st.session_state["vykresli_otazku_fn"](
                "5.3.1",
                "Trenažér daně v praxi: Zvol si životní situaci a napiš 2-3"
                " klíčové otázky k daním, které si musíte zjistit.",
                "5",
                st.session_state.get("ulozene_odpovedi", {}),
            )
    ...
    # =========================================================================
        ...
        st.markdown(
            ...
            "🌐 <b>Moderní hook:</b> Tričko z Temu, mobil navržený v USA, čip"
            " z Tchaj-wanu, kompletace ve Vietnamu, doprava přes Suez a prodej"
            " v Česku. Globalizace znamená, že věci, které denně používáš,"
            " nevznikají „v jedné zemi“, ale v síti firem, států, dopravních"
            " cest, dat a pravidel."
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander(
            "Proč tričko z druhého konce světa může stát méně než lístek na"
            " MHD?"
        ):
            st.write(
        ...
        st.divider()
        st.markdown(
            "### 4.2 Volný obchod, protekcionismus a obchodní bariéry"
        )
        st.markdown("""
        ...
        st.warning(
            "🧱 **Obchodní bariéry:** **clo** (daň na dovážené zboží), **kvóta**"
            " (omezení množství dovozu), **embargo** (zákaz obchodu s určitou"
            " zemí nebo zbožím), **technické normy** (pravidla kvality,"
            " bezpečnosti, původu nebo složení výrobku)."
        )

        with st.expander(
            "Protekcionismus v praxi: cla na čínské elektromobily"
        ):
            st.write(
                "Pokud EU zavede cla na levnější čínské elektromobily, může"
                " tím chránit evropské automobilky a pracovní místa. Zároveň ale"
                " může zdražit auta zákazníkům a zpomalit přechod k"
                " elektromobilitě. Debatní otázka: Má stát chránit domácí"
                " průmysl, i když tím zákazníkům zdraží výrobky?"
            )

        with st.expander("Levné balíčky z Asie a rušení výjimek"):
            st.write(
                "Mnoho států řeší, jak nastavit pravidla pro drobné zásilky z"
                " mimoevropských e-shopů. Pokud jsou malé balíčky zvýhodněné,"
                " může to podporovat levné dovozy, ale zároveň znevýhodnit"
                " domácí firmy a ztížit kontrolu kvality, bezpečnosti i daní."
            )
        ...
        c_col1.metric(
            "Čínské auto s clem", f"{cena_cina:,} Kč".replace(",", " ")
        )
        ...
        st.divider()
        st.markdown(
            "### 4.3 Globální dodavatelské řetězce a zranitelnost ekonomiky"
        )
        st.write(
            "Moderní ekonomika funguje v síti dodavatelů. Jeden výrobek může"
            " projít přes desítky zemí. Firmy často používají systém"
            " **Just-in-Time**, kdy se neskladuje mnoho zásob a díly přicházejí"
            " přesně tehdy, kdy jsou potřeba. Výhoda: nižší náklady, rychlejší"
            " výroba. Riziko: když se zasekne doprava, čip nebo energie, zastaví"
            " se celý řetězec."
        )
        ...
        with st.expander("Když se zasekne Suez nebo Rudé moře"):
            st.write(
                "Suezský průplav a další úzká místa světové dopravy jsou jako"
                " „dopravní tepny“ globalizace. Když se zablokují, lodě musí"
                " plout oklikou, doprava trvá déle, pojištění a palivo zdraží a"
                " část nákladů se promítne do cen zboží."
            )

        st.divider()
        st.markdown(
            "### 4.4 Evropská unie a jednotný vnitřní trh: náš domácí prostor"
        )
        st.write(
            "Česká republika není ekonomický ostrov. Je součástí **Evropské"
            " unie** a jejího **jednotného vnitřního trhu**, který patří mezi"
            " největší obchodní prostory na světě."
        )
        ...
        **Čtyři svobody EU:**
        ...
        with st.expander("EU jako silný regulátor: USB-C, GDPR a AI Act"):
            st.write(
                "EU dokáže ovlivnit globální firmy, protože evropský trh je"
                " velký a bohatý. Proto může prosadit pravidla pro nabíječky,"
                " ochranu osobních údajů, digitální platformy nebo umělou"
                " inteligenci. Debata: Chrání EU spotřebitele, nebo příliš"
                " reguluje inovace?"
            )

        with st.expander("Mělo by Česko přijmout euro?"):
            st.write(
                "**Argumenty PRO:** firmám by odpadlo měnové riziko při"
                " obchodování v eurozóně, lidem by odpadla část směn a kurzových"
                " poplatků při cestování, Česko by bylo hlouběji propojené s"
                " eurozónou.\n\n**Argumenty PROTI:** Česko by ztratilo vlastní"
                " měnovou politiku ČNB, přijetí eura může být politicky i"
                " psychologicky citlivé, společná měna nemusí všem ekonomikám"
                " vyhovovat stejně."
            )
        ...
        st.divider()
        st.markdown(
            "### 4.5 Klimatická krize, udržitelný rozvoj a mezinárodní"
            " instituce"
        )
        st.write(
            "Hospodářský růst naráží na limity planety. Ekonomika řeší otázku,"
            " jak vyrábět, dopravovat a spotřebovávat tak, aby růst neznamenal"
            " ničení zdrojů, klimatu a zdraví lidí. **Trvale udržitelný"
            " rozvoj:** Rozvoj, který uspokojuje potřeby současné generace,"
            " aniž by ohrozil možnost budoucích generací uspokojovat jejich"
            " vlastní potřeby."
        )
        ...
        with st.expander("Greenwashing: když se firma tváří zeleněji, než je"):
            st.write(
                "Greenwashing znamená, že firma používá ekologickou rétoriku"
                " hlavně marketingově, ale skutečné dopady jejího podnikání se"
                " zásadně nemění. Kontrolní otázky: Má firma konkrétní data? Je"
                " jasné, co měří? Jsou cíle ověřitelné? Nezakrývá jeden „zelený“"
                " produkt celkově problematický byznys?"
            )

        st.divider()
        st.markdown(
            "### 4.6 Budoucnost práce a financí v globálním světě"
        )
        st.write(
            "Globalizace už není jen o kontejnerech a továrnách. Týká se také"
            " práce, dat, online služeb, kapitálu a digitálních měn."
        )
        ...
        st.info(
            "🏝️ **Digitální nomád není mimo systém:** To, že někdo pracuje z"
            " Bali, Portugalska nebo Španělska pro klienta z USA, neznamená, že"
            " „nemusí řešit stát“. Musí řešit daňovou rezidenci, zdravotní"
            " pojištění, sociální pojištění, víza, pracovní povolení a pravidla"
            " země, kde skutečně žije i odkud má příjmy."
        )
        ...
        **Otázka:** Komu z tohoto řetězce zůstane největší marže — těžaři surovin, továrně, dopravci, značce, platformě nebo prodejci? Proč?
        ...
        st.success(
            "✅ **Co si zapamatovat:** Globalizace přinesla levnější zboží,"
            " větší výběr, nové příležitosti a propojení světa. Zároveň ale"
            " vytvořila závislosti, zranitelné řetězce, ekologické dopady a"
            " otázku, kdo skutečně nese náklady levné spotřeby."
        )
    ...
    # =========================================================================
        ...
        st.markdown(
            "<div class='box-green'>"
            "🌱 <b>Moderní hook:</b> Firma dnes nestačí hodnotit jen podle"
            " toho, kolik vydělá. Investoři, banky, zákazníci i zaměstnanci se"
            " stále častěji ptají: Jak firma zachází s lidmi? Jakou má uhlíkovou"
            " stopu? Nezneužívá dodavatele? Není její „zelená“ reklama jen"
            " greenwashing?"
            "</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "### 5.1 Udržitelný rozvoj: ekonomika, která nespotřebuje"
            " budoucnost"
        )
        st.write(
            "Udržitelný rozvoj znamená takový způsob výroby, spotřeby a života,"
            " který uspokojuje potřeby současné generace, aniž by ohrozil"
            " možnosti budoucích generací. Nejde jen o ochranu přírody."
            " Udržitelnost má tři propojené roviny:"
        )
        ...
        st.write(
            "**ESG** je zkratka pro **Environmental, Social, Governance**."
            " Používá se při hodnocení firem, investic a rizik. Nejde jen o"
            " morálku nebo reklamu — ESG může ovlivnit, zda firma získá úvěr,"
            " investici, veřejnou zakázku, dobré zaměstnance nebo důvěru"
            " zákazníků."
        )
        ...
        with st.expander("Proč banku zajímá, jestli je firma ekologická?"):
            st.write(
                "Banka nechce půjčit peníze firmě, která může za pár let"
                " narazit na pokuty, drahé energie, reputační skandál nebo"
                " ztrátu zákazníků. ESG proto funguje i jako řízení rizik. Firma"
                " se špatným ESG profilem může mít dražší financování nebo"
                " horší přístup k investorům."
            )

        st.divider()
        st.markdown(
            "### 5.3 Greenwashing: když zelená reklama zakrývá realitu"
        )
        st.write(
            "**Greenwashing** znamená, že firma působí ekologicky nebo"
            " odpovědně hlavně v reklamě, ale její skutečné dopady se zásadně"
            " nemění. Často používá vágní slova jako „eco“, „green“,"
            " „natural“, „planet friendly“ bez jasných dat."
        )
        ...
        * nemá nezávislé ověření nebo srovnání.
        ...
        with st.expander("Příklad: „eko“ kolekce ve fast fashion"):
            st.write(
                "Značka může propagovat malou kolekci z recyklovaného materiálu,"
                " ale zároveň každý týden vyrábět obrovské množství levného"
                " oblečení, které rychle končí v odpadu. To neznamená, že"
                " recyklovaný materiál je špatně — problém je, pokud malý"
                " zelený prvek zakrývá celkově neudržitelný model."
            )
        ...
        st.divider()
        st.markdown(
            "### 5.4 Cirkulární ekonomika: od „vyrobit–použít–vyhodit“ k oběhu"
        )
        st.write(
            "Tradiční lineární model ekonomiky funguje jako **vyrobit → použít"
            " → vyhodit**. Cirkulární ekonomika se snaží, aby materiály a"
            " výrobky zůstávaly v oběhu co nejdéle."
        )
        ...
        """)
        st.info(
            "♻️ **Příklady cirkulární ekonomiky:** opravitelné telefony, vratné"
            " obaly, second-hand, repasovaná elektronika, knihovny věcí, sdílená"
            " auta, recyklace materiálů, prodloužená záruka, právo na opravu."
        )

        st.divider()
        st.markdown("### 5.5 ESG v dodavatelských řetězcích")
        st.write(
            "Firma nenese odpovědnost jen za svou kancelář, ale i za dodavatele:"
            " kde vznikly suroviny, za jakých podmínek se kompletovaly"
            " komponenty a kolik emisí stála doprava."
        )
        ...
        st.divider()
        st.markdown(
            "### 5.6 Odpovědný spotřebitel: hlasuješ peněženkou, ale ne všechno"
            " je na tobě"
        )
        st.write(
            "Spotřebitel může ovlivnit trh tím, co kupuje, jak často nakupuje a"
            " jaké značky podporuje. Zároveň ale není fér svalit veškerou"
            " odpovědnost jen na jednotlivce. Pravidla nastavují i státy, EU,"
            " firmy, investoři a mezinárodní dohody."
        )

        with st.expander(
            "Měl by být výrobek dražší, když lépe zahrnuje ekologické a"
            " sociální dopady?"
        ):
            st.write(
                "**Argument PRO:** Levné zboží často neobsahuje všechny"
                " skutečné náklady — emise, odpad, vodu nebo špatné pracovní"
                " podmínky. Vyšší cena může lépe odrážet realitu.\n\n**Argument"
                " PROTI:** Vyšší ceny mohou dopadnout hlavně na nízkopříjmové"
                " domácnosti. Udržitelnost se pak může stát „luxusem pro"
                " bohaté“.\n\n**Vyvážený pohled:** Cílem není jen zdražit"
                " všechno, ale nastavit pravidla, podporovat inovace, opravy,"
                " dostupné alternativy a férové informace pro zákazníka."
            )

        st.markdown(
            "<div class='box-green'><strong>🔗 Užitečné zdroje pro práci ve"
            " třídě:</strong><br>- <a href='https://faktaoklimatu.cz/'"
            " target='_blank'>Fakta o klimatu</a> — srozumitelná data a grafiky"
            " ke klimatu.<br>- <a href='https://faktaoklimatu.cz/kalkulacka'"
            " target='_blank'>Kalkulačka uhlíkové stopy Fakta o klimatu</a> —"
            " orientační propojení životního stylu a emisí.<br>- <a"
            " href='https://sdgs.un.org/goals' target='_blank'>OSN — Cíle"
            " udržitelného rozvoje</a> — globální rámec pro udržitelný"
            " rozvoj.</div>",
            unsafe_allow_html=True,
        )
        ...
        st.write(
            "Vyber jednu značku, kterou znáš — oblečení, elektronika,"
            " kosmetika, banka, e-shop nebo potraviny. Zkus najít: Co značka"
            " tvrdí o ekologii? Uvádí konkrétní data, nebo jen slogany? Řeší"
            " pracovní podmínky a dodavatele? Má informace o opravách,"
            " recyklaci nebo obalech? Co by mohlo být greenwashing?"
        )
        znacka_nazev = st.text_input(
            "Název značky:", value="H&M", key="k5_5_6_znacka"
        )
        znamka = st.select_slider(
            "Dej značce známku ESG:",
    ...
    # =========================================================================
        ...
        st.write(
            "Vyberte jednu studii do dvojice nebo skupiny. Nejprve popište"
            " ekonomický problém, potom určete roli státu, daní, trhu,"
            " globalizace a odpovědnosti firem. Nakonec navrhněte řešení a"
            " obhajte ho před třídou."
        )
        ...
        with tab_studie1:
            ...
            st.write(
                "**Situace:** Student si koupí tričko z fast fashion e-shopu za"
                " 99 Kč. Na první pohled je to výhodný nákup. Tričko ale"
                " vzniklo v globálním dodavatelském řetězci: bavlna, barvení"
                " látky, šití, balení, doprava, sklad, reklama, platforma a"
                " doručení až ke dveřím."
            )
            with st.expander("Možné řešení"):
                st.write(
                    "Levná cena může vzniknout díky velkému objemu výroby,"
                    " nízkým mzdovým nákladům, efektivní logistice a silnému"
                    " tlaku na dodavatele. Skryté náklady mohou být emise z"
                    " dopravy, spotřeba vody, textilní odpad nebo horší pracovní"
                    " podmínky. Stát a EU mohou nastavovat pravidla pro"
                    " bezpečnost výrobků, férovou soutěž, informace pro"
                    " spotřebitele nebo ekologické standardy. Spotřebitel může"
                    " omezit impulzivní nákupy, využít second-hand, kupovat"
                    " kvalitnější věci nebo se ptát na původ výrobku."
                )
        ...
        with tab_studie2:
            ...
            st.write(
                "**Situace:** Student natáčí videa na TikTok, občas dostane"
                " barter od značky, prodává digitální šablony a jednou za čas"
                " mu přijde příjem z affiliate odkazu. Říká si: „Je to jen"
                " bokovka, daně řešit nemusím.“"
            )
            with st.expander("Možné řešení"):
                st.write(
                    "I barter může mít ekonomickou hodnotu, pokud je poskytnut"
                    " výměnou za propagaci. Pokud se činnost opakuje, má záměr"
                    " vydělávat a student ji organizuje soustavně, může jít o"
                    " zdanitelný příjem nebo podnikání. Student by měl ověřit,"
                    " zda musí podat daňové přiznání, zda potřebuje živnostenské"
                    " oprávnění, jak evidovat příjmy a výdaje, jak řešit"
                    " zdravotní a sociální pojištění a jak dokládat nepeněžní"
                    " plnění. Pointa není studenty strašit, ale ukázat, že"
                    " „internetový příjem“ není mimo pravidla."
                )
        ...
        with tab_studie3:
            ...
            st.write(
                "**Situace:** Menší obec má rozhodnout, jak využije 10 milionů"
                " Kč. Část obyvatel chce opravit silnici a chodníky, část chce"
                " podpořit školu a volnočasové aktivity, část požaduje snížení"
                " místních poplatků a podnikatelé chtějí lepší parkování u"
                " náměstí."
            )
            with st.expander("Možné řešení"):
                st.write(
                    "Oprava silnic a chodníků souvisí s alokační funkcí a"
                    " veřejnou infrastrukturou. Podpora školy může být"
                    " pozitivní externalita, protože vzdělání prospívá nejen"
                    " jednotlivci, ale celé komunitě. Sociální pomoc má"
                    " redistribuční funkci. Podpora podnikání může zvýšit"
                    " zaměstnanost a služby v obci, ale musí být férová a"
                    " transparentní. Obec by měla ukázat kritéria rozhodování,"
                    " očekávané dopady, náklady a důvody, proč nelze splnit"
                    " všechny požadavky najednou."
                )
            ...
            m_skola = st.slider(
                "🏫 Školství (mil. Kč):", 0.0, 10.0, 3.0, step=0.5, key="cs3_skola"
            )
            m_soc = st.slider(
            ...
            if st.button(
                "Uložit rozdělení obecního rozpočtu 💾", key="btn_cs3_rozpocet"
            ):
                roz_obec_data = (
        ...
        with tab_ukol:
            ...
            st.write(
                "Vyberte jeden běžný produkt a zkuste odhadnout, jaké země,"
                " firmy a dopravní kroky mohly být součástí jeho cesty k"
                " zákazníkovi."
            )
